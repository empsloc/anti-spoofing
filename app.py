...
def initialize_data():
    os.makedirs(VISITOR_DB, exist_ok=True)
    if not os.path.exists(os.path.join(VISITOR_DB, file_db)):
        df = pd.DataFrame(columns=['Name'] + [f'v{i}' for i in range(512)])
        df.to_csv(os.path.join(VISITOR_DB, file_db), index=False)
...
def main():
    ...
    st.markdown(
        ...
        <style>
            ...
            .main-title { text-align: center; font-size: 36px; font-weight: bold; color: #003366; }
            [data-testid="stSidebar"] { background-color: white !important; }
            .stButton button { background-color: #003366; color: white; border-radius: 10px; }
        </style>
        """,
    ...
    with st.sidebar:
        ...
        selected_menu = option_menu("Main Menu",
            ['Visitor Validation', 'View Visitor History', 'Add to Database'],
            icons=['camera', "clock-history", 'person-plus'],
    ...
    if selected_menu == 'Visitor Validation':
        st.subheader("Visitor Validation")
        visitor_id = uuid.uuid1()
        img_file_buffer = st.camera_input("Take a picture")
        ...
        if img_file_buffer is not None:
            ...
            with open(os.path.join(VISITOR_HISTORY, f'{visitor_id}.jpg'), 'wb') as file:
                file.write(img_file_buffer.getbuffer())
                st.success('Image Saved Successfully!')
                st.image(Image.open(img_file_buffer), caption="Captured Image", use_column_width=True)
        ...
        if os.path.exists(f_p):
            ...
            st.warning("No visitor history found!")
    
    elif selected_menu == 'Add to Database':
        st.subheader("Add Visitor to Database")
        name = st.text_input("Enter Visitor Name")
        if st.button("Add to Database"):
            df = pd.DataFrame([[name] + [0] * 512], columns=['Name'] + [f'v{i}' for i in range(512)])
            df.to_csv(os.path.join(VISITOR_DB, file_db), mode='a', header=False, index=False)
